...
- On the [App Store](https://apps.apple.com/app/id6774141263) (current version 1.0.0)

## Frequently asked questions

### Where are my photos and recordings stored?

Photos, audio, memos and drafts are stored only on your device. The only things sent to a server are the text and photo information used to generate an outline. Photo files themselves are never sent.

### Is the capture location sent?

A place name is passed to outline generation only when "Use the capture location" is on for that draft. With it off, a photo's location never leaves your device. Latitude and longitude are never sent.
...
The monthly generation allowance and the exporting that came with AI Plus stop. The free quota returns to whatever was left before you subscribed. To keep exporting afterwards, consider the one-time or yearly export plan.
...
- The one-time and yearly export plans keep exporting available after the initial purchase.
